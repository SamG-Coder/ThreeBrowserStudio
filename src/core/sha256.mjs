...
/** Sync SHA-256 hex digest. Node `contentHash` still prefers builtin crypto. */
export function sha256Hex(bytes) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      const next = words[index - 15];
      const last = words[index - 2];
      const s0 = rotr(next, 7) ^ rotr(next, 18) ^ (next >>> 3);
      const s1 = rotr(last, 17) ^ rotr(last, 19) ^ (last >>> 10);
