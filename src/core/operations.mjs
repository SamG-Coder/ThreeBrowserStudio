...
const OPERATION_KEYS = new Map([
  ...
  ['camera.frame', new Set(['type', 'op', 'cameraId', 'bounds', 'targetIds', 'aspect', 'padding', 'distanceScale', 'direction', 'minHeight', 'lockPreviewAspect'])],
...
function applyCameraFrame(draft, operation, aliases) {
  ...
  try {
    framed = solveCameraFrame({
      ...
      distanceScale: operation.distanceScale,
