...
function splitByProjectedCurve(mesh, curve) {
  ...
  const enclosed = []; const remainder = [];
  const triangleClass = [];
  for (let triangleId = 0; triangleId < mesh.indices.length / 3; triangleId += 1) {
  ...
  if (enclosed.length === 0 || remainder.length === 0) {
  ...
  for (let triangleId = 0; triangleId < triangleClass.length; triangleId += 1) {
    ...
    for (let edge = 0; edge < 3; edge += 1) {
      ...
      classes.add(triangleClass[triangleId]); edgeClasses.set(key, classes);
    }
  }
  ...
  return {
    enclosedRecipe: compactRecipe(mesh, enclosed),
    enclosedWorldRecipe: compactRecipe(mesh, enclosed, mesh.worldPositions),
    remainderRecipe: compactRecipe(mesh, remainder),
    enclosedTriangleCount: enclosed.length,
    remainderTriangleCount: remainder.length,
    boundaryEdgeCount,
    imprinted: true,
  };
}
