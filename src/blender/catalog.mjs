...
const entries = [
  ...
  capability({
    ...
    runtimeNotes: `Array, mirror, and pattern lower to bounded instance transforms. ${LIVE_EDITABLE_MESH_GEOMETRY_MODIFIERS.length} deterministic editable-mesh modifiers evaluate as derived geometry without mutating the shared base resource. Static smooth, simpleDeform, displacement, solidify, and simple/Loop subdivision evaluate on canonical editable vertices before seam-preserving UV triangulation in both the viewport and GLB export. Manifold shells retain corner layers/materials and close boundary rims with independent strip UVs; sharp edges and persistent normalized crease weights guide subdivision. Topology changes must precede weightedNormal, edgeSplit, and Ocean. A bakeBoundary stops live downstream evaluation instead of silently approximating it. Blender 5.2's complete RNA inventory remains queryable; authored render-enable flags are preserved but do not claim render/evidence parity.`,
  ...
  capability({
    ...
    supportedSubset: ['base color', 'roughness', 'metalness', 'emissive', 'opacity', 'clearcoat', 'transmission', 'multiple face material slots', 'canonical image texture binding', 'subsurface tint/scale approximation', 'toon/basic/standard/physical families', 'Blender-shaped live shader graphs', 'semantic automotive paint/rubber/brushed metal/glass/emissive lens/fabric/organic skin looks'],
  ...
  capability({
    ...
    supportedSubset: ['sRGB/non-color declarations', 'UV and generated coordinates', 'checker/gradient/noise/Voronoi/Wave/ramp/math evaluation', 'bounded deterministic CPU bake job', 'canonical raster GPU binding', 'live procedural TSL', 'bounded UV-space texture-paint strokes with deterministic smoothing, spacing, pressure, and symmetry'],
    unsupportedSubset: ['external image-file decoding through MCP', 'UDIM', 'layered tiled paint documents', 'height-map job output'],
  ...
  capability({
    ...
    supportedSubset: ['directional', 'point', 'spot', 'ambient', 'hemisphere', 'rectangular area-light authoring with an explicit safe native point approximation', 'color', 'intensity', 'cast shadow', 'position and direction through transforms', 'softbox and rim-strip rig presets'],
    unsupportedSubset: ['true rectangular WebGPU emission', 'light node trees', 'IES profiles', 'light linking', 'Cycles-specific sampling controls'],
  ...
  capability({
    ...
    status: 'partial',
    canonicalRepresentation: 'Scene settings for background color, procedural environment illumination, and fog.',
    mcpWorkflow: ['three_studio_apply: scene.settings.patch', 'three_studio_render'],
    runtimeNotes: 'Solid background, fog, and optional bounded procedural studio/sky image-based lighting compile. Environment illumination is independent of the visible background; node-based worlds and external HDR images remain unsupported.',
    supportedSubset: ['linear background color', 'scene fog document', 'procedural studio and sky reflection environments'],
    unsupportedSubset: ['world shader graph', 'external HDRI images', 'physical atmospheric scattering', 'world volume shader', 'light probes'],
  ...
  capability({
    ...
    status: 'partial',
    canonicalRepresentation: 'Ocean is an ordered typed geometry modifier. Runtime rigid bodies and colliders are typed entity components evaluated in the bounded controller fixed step.',
    mcpWorkflow: [
    ...
    unsupportedSubset: [
      ...
      'soft body', 'cloth', 'fluid', 'force fields',
  ...
  capability({
    ...
    supportedSubset: ['multiple named UV layers', 'exact per-corner UV edits and seams', 'active UV layer', 'transform UVs', 'planar/cylindrical/spherical/box projection', 'raster and graph sampling'],
