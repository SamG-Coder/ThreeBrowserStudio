...
- A **Prompt** tab (browser only) for connecting HTTP chat APIs through an
  abstract provider catalog. Bearer tokens live in `localStorage` only as
  AES-GCM ciphertext; a PIN unlocks them for this tab session.
...
The browser Prompt tab talks to the nine `three_studio_*` names through a
